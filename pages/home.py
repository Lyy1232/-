...
def render():
    ...
    # ═══════ ROW 1: Top-line KPIs ═══════
    st.markdown('<p style="font-size:11px;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;font-weight:600">核心指标</p>', unsafe_allow_html=True)
    ...
    with r2l:
        ...
        for site in sites:
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border-light)">
              <span style="font-weight:700;font-size:13px;width:48px;color:var(--text)">{site['name']}</span>
              ...
              <span style="font-size:11px;color:var(--text-3);width:100px;text-align:right">¥{site['cost_low']}–¥{site['cost_high']}</span>
            </div>""", unsafe_allow_html=True)
        st.markdown('<div style="display:flex;justify-content:space-between;font-size:9px;color:var(--text-4);margin-top:4px;padding:0 48px"><span>¥10</span><span>¥15</span><span>¥20</span><span>¥25</span><span>¥30</span></div>', unsafe_allow_html=True)
        ...
        for c in clusters[:6]:
            ...
            <div style="padding:6px 0;border-bottom:1px solid var(--border-light)">
              <div style="display:flex;justify-content:space-between;margin-bottom:3px">
                ...
                <span style="font-size:11px;color:var(--text-3)">{c['count']}站 · {c['throughput']:,} kg/天</span>
              </div>
    ...
    with r3l:
        ...
        for _, r in df_cov.iterrows():
            ...
            rows_html += f"""
            <tr>
              <td style="font-weight:600;color:var(--text)">{r['基地']}</td>
              <td>{r['覆盖站数']} 站</td>
              <td>{r['日需求(kg)']:,} kg</td>
        ...
        <table style="width:100%;font-size:12px;border-collapse:collapse">
          <thead><tr style="color:var(--text-3);font-size:10px;text-transform:uppercase;letter-spacing:0.5px">
            <th style="text-align:left;padding:8px;border-bottom:2px solid var(--border)">基地</th>
            <th style="text-align:left;padding:8px;border-bottom:2px solid var(--border)">覆盖</th>
            <th style="text-align:left;padding:8px;border-bottom:2px solid var(--border)">日需求</th>
            <th style="text-align:left;padding:8px;border-bottom:2px solid var(--border)">供给能力</th>
            <th style="text-align:left;padding:8px;border-bottom:2px solid var(--border)">状态</th>
          </tr></thead>
    ...
    with r3r:
        ...
        for icon, title, desc, target in nav_items:
            ...
            onclick="document.getElementById('dummy').click()">
              ...
              <div style="flex:1"><div style="font-weight:600;font-size:13px;color:var(--text)">{title}</div>
              <div style="font-size:11px;color:var(--text-3)">{desc}</div></div>
            </div>""", unsafe_allow_html=True)
    ...
    with r4l:
        st.markdown('<p style="font-size:11px;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;font-weight:600">基地状态</p>', unsafe_allow_html=True)
    ...
    with r4r:
        st.markdown('<p style="font-size:11px;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;font-weight:600">数据状态</p>', unsafe_allow_html=True)
        st.markdown(f"""
        <div class="data-card">
